...
use Common\UtilBundle\ActionBar\ActionBar;
...
class Datatable
{
    ...
    const FILTER_LABEL = 'sFilterLabel';

    const FILTER_OPTIONS = 'aFilterOptions';

    const MDATA_PROP = 'mDataProp';
    ...
    /**
     * Column property: sorting
     */
    const COLUMN_ALLOWED_SORTING = 'asSorting';

    /**
     * Column property: sorting
     */
    const COLUMN_SORTING = 'aaSorting';
    ...
    /**
     * @var string Not available fallback
     */
    protected $notAvailableFallback;
    ...
    /**
     * @var array A map with the column that need be ordered and its direction
     */
    protected $sortingParameters = array();
    ...
    public function __construct(array $request, EntityRepository $repository, ClassMetadata $metadata, EntityManager $em, $serializer, $serverSideControl)
    {
        ...
        if (sizeof($this->request) > 0) {
            $this->echo = isset($this->request['sEcho'])
                ? $this->request['sEcho']
                : '';
            $this->search = isset($this->request['sSearch'])
                ? $this->request['sSearch']
                : '';
            $this->offset = isset($this->request['iDisplayStart'])
                ? $this->request['iDisplayStart']
                : 0;
            $this->amount = isset($this->request['iDisplayLength'])
        ...
        $this->offset = !is_null($this->offset) ? $this->offset : 0; // fallback value
        $this->amount = !is_null($this->amount) ? $this->amount : 10000; // fallback value
    }
     ...
     *
     * @param string $key A dotted-notation property format key used by DQL to fetch your object. Use the property from the object that you provided to getDatatable() method
     ...
     * @param array $filterableOptions (optional) An array with label/value object pairs used as the available options at 'bFilterable' property from datatables aoColumns;
     *
     * @return self
     */
    public function addColumn($key, $rawOptions = null, $filterCallback = null, array $filterableOptions = array())
    {
        ...
        $rawOptions['bFilterable'] = false;
        if (count($filterableOptions) > 1) {
            $rawOptions['bFilterable'] = array(
                self::FILTER_LABEL => $rawOptions[self::COLUMN_TITLE],
                self::FILTER_OPTIONS => $filterableOptions,
                self::MDATA_PROP => $key
            );
        }
 ...
 */
    ...
    /**
     * Adds a function to process rows return response
     *
     * @param string $key Your key name
     * @param callback $callback The function used to apply to each object from query builder
     * @throws \Exception If the callback is not a callable function
     */
    public function addRowPass($callback)
    {
        if (!is_callable($callback)) {
            throw new \Exception(sprintf("The first argument of \"%s\" method must be a callable function", __FUNCTION__));
        }
        if (!isset($this->callbacks['rowPass'])) {
            $this->callbacks['rowPass'] = array();
        }
        $this->callbacks['rowPass'][] = $callback;
    }
    ...
    public function setWhere(QueryBuilder $qb)
    {
        ...
        for ($i = 0; $i < count($this->parameters); $i++) {
            if (isset($this->request['bSearchable_' . $i]) && $this->request['bSearchable_' . $i] == "true" && $this->request['sSearch_' . $i] != '') {
                ...
                if (isset($this->request['bRegex_' . $i]) && $this->request['bRegex_' . $i] == true) {
                    $andExpr->add(
                        $qb->expr()->in(
                            $this->associations[$i]['fullName'],
                            explode('|', $this->request['sSearch_' . $i])
                        )
                    );
                } else {
                    $andExpr->add(
        ...
        foreach ($items as $item) {
            ...
            for ($i = 0 ; $i < count($this->parameters); $i++) {
                ...
                if (!$this->associations[$i]['containsCollections']) {
                    ...
                    $item[$parameterKey] = isset($item[$parameterKey])
                        ? $item[$parameterKey]
                        : null; // Inject missing parameters (null as a pattern to column filtering)
                    $this->applyColumnFiltering($parameterKey, $item[$parameterKey]); // Apply column filtering if needed
                    $item[$parameterKey] = is_null($item[$parameterKey])
                        ? $this->notAvailableFallback // If the developer didn't filtered, replace by the fallback value
                        : $item[$parameterKey];
                ...
                while ($field = array_shift($fields)) {
                    ...
                    if (!empty($fields) && !$this->isAssocArray($rowRef)) {
                        ...
                        // Missing fields treatment
//                        $rowRef = (sizeof($rowRef) == 0) ? $rowRef : null; // Inject missing parameters (null as a pattern to column filtering)
                        $this->applyColumnFiltering($parameterKey, $rowRef);
                        $rowRef = (sizeof($rowRef) == 0)
                            ? $this->notAvailableFallback // If the developer didn't filtered, replace by the fallback value
                            : $rowRef;
                    }
                }
            ...
            if (isset($this->callbacks['rowPass'])) {
                foreach ($this->callbacks['rowPass'] as $rowPassCallback) {
                    $item['DT_RowExtra'] = $rowPassCallback($item);
                }
            }
        ...
        if ($this->serverSideControl) {
            ...
            if (sizeof($this->sortingParameters)) $outputHeader[self::COLUMN_SORTING][] = $this->sortingParameters;
        }
    ...
    {
        if (isset ($this->datatable['aaData'])) {
            foreach ($this->datatable['aaData'] as $rowData) {
                // Extract all the available actions from the control data if an action bar exists
                if (isset($rowData['DT_RowExtra']) && isset($rowData['DT_RowExtra'][ActionBar::CONTROL_DATA])) {
                    $controlData = $rowData['DT_RowExtra'];
                    $this->extractAvailableActions($controlData);
                }
            }
        }

        if (count($this->availableActions) > 0) {
            $this->datatable['aoCustomVars'] = array_merge(
                $this->datatable['aoCustomVars'],
    ...
    /**
     * Extract all the available actions from the control data
     *
     * @param array $controlData
     *
     * @return array
     */
    public function extractAvailableActions(array $controlData)
    {
        $controlData = $controlData[ActionBar::CONTROL_DATA];
        foreach ($controlData as $group) {
            foreach ($group as $groupItem) {
                $action = isset($groupItem[ActionBar::ACTION]) ? $groupItem[ActionBar::ACTION] : null;
                if ($action && array_search($action, $this->availableActions) === false) {
     ...
     */
    protected function getAoCustomVars($formatted = false)
    {
        if (!$formatted) {
    ...
    /**
     * Sets a fallback string to be replaced in the columns where no value was returned by the query builder but the
     * datatables.js needs to render something to the user
     *
     * @param string $notAvailableFallback
     */
    public function setNotAvailableFallback($notAvailableFallback)
    {
        $this->notAvailableFallback = $notAvailableFallback;
    }
    ...
    public function setSorting($parameters)
    {
